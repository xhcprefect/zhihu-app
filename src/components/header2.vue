<template>
  <div class="back">
    ...
    <span class="iconfont icon-shoucang icon3" :style="{color:color}" @click="sc"></span>
  ...
  methods: {
    ...
    sc() {
      ...
      this.axios({
        ...
        localStorage.setItem("list", JSON.stringify(this.articleList));
        this.$store.commit("setArticleList", res.data);
      });
      this.color = "#FFD700";
      if ((this.$store.state.setScId = this.id)) {
        this.$store.commit("setColor", "#FFD700");
      }
    }
  },
  mounted() {
